...
class DialogColor {
  ...
  //     BuildContext context, ColorShoe itemUpdate) async {
  //   ColorShoe color = itemUpdate;
  ...
  Future<void> showDialogColor(BuildContext context,
      {ColorShoe? itemUpdate}) async {
    DialogCommon().showDialogWithBody(context,
        height: 320,
        title: itemUpdate != null
            ? 'Sửa màu sắc có id: ${itemUpdate.id}'
            : 'Thêm Màu Sắc',
        bodyDialog: ColorInputDialog(
            colorShoe: itemUpdate,
            updateColor: (colorShoe) => viewModel.updateColor(colorShoe),
            addColor: (colorShoe) => viewModel.addColor(colorShoe)));
  }
}
